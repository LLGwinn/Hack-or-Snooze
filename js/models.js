...
class Story {
  ...
  getHostName() {
    ...
    const urlToParse = new URL(this.url);
...
class StoryList {
  ...
  async addStory( user, {username, title, author, url} ) {
    ...
    const response = await axios({
      ...
      url: `${BASE_URL}/stories`,
      data: { token,
              story: {username, title, author, url}
      }
    });
...
class User {
  ...
  /** add story to user favorites and update API */
  async addFavorite(story) {
    this.favorites.push(story);
    await this._addOrRemoveFavorite("add", story);
  }

  /** update user favorites to not include story and update API */
  ...
  /** remove story from user's ownStories */
